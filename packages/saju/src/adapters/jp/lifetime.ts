 ...
 * 진태양시(真太陽時) 보정에 매우 민감하므로 `trueSolar` 메타로 정확도를 표기.
 *
 * NOTE: 일본 추명학은 용신(用神) 개념을 사용하지 않는다 — `yongshin: undefined`
 * 는 의도적 omit (extendedTypes.ts:38 명시 사양). cn-mangpai 의 v0.1 미적용 TODO
 * 와 의미가 다르므로 `cautions` 에 별도 표기하지 않는다. Phase 5 compose 단계의
 * `yongshinConflicts` 산출 시 silent skip 대상.
 *
