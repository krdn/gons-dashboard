...
describe("buildLifetimeJp", () => {
  it("1967-03-29 → school='jp', 진태양시 보정 정확도 표기 + yongshin 의도적 omit", () => {
    ...
    expect((frame.schoolSpecific as { system?: string }).system).toContain("추명학");
    // 일본 추명학은 용신 개념 미사용 — undefined 가 의도적 omit (extendedTypes.ts:38)
  ...
  it("hourKnown=false → 시주 미상 정확도 ⚠ 표기", () => {
    ...
    const frame = buildLifetimeJp(chart, { trueSolarMinutesOffset: 0, hourKnown: false });
