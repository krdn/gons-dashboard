...
삼국(tri-nation) v0.2 narrative 의 cache-or-generate 를 묻는 깊은 모듈 (`createNarrativeCache<Frame, Sections, Result>` factory, `shared/lib/saju/`). **캐시-리딩 모듈과 별개** — 캐시-리딩 모듈은 `callSajuLlm` (단일 호출, 재시도는 caller 책임) 기반의 *레거시 saju 리딩* (섹션 리딩/세운/일진) 용이고, 캐시-narrative 모듈은 `analyzeStructured` + **ZodError 1회 재시도를 factory 가 소유** 하는 *삼국 학파별 narrative* (lifetime/yearly/monthly/daily × 한/중-자평/중-맹파/일) 용이다. 재시도 책임 위치가 반대라 한쪽이 다른 쪽의 조상이 아니다. factory 는 frameHash + null 자가치유 가드 (sections·schoolSpecific 둘 다) + `assertSajuBudgetOk` + retry 루프 + `analyzeStructured`/`normalizeUsage`/`computeKrw` + `logSajuSpend` (validated outputs only) **정책 시퀀스** 를 묻는다. cache I/O 는 `createSajuTriCache` 와 동일하게 **`findCached`/`insertCache` 콜백으로 caller 에 위임** — DB-shape 변이(테이블·추가 키 컬럼·result envelope 추가 필드 `forDate`/`targetYear`/`targetMonth`)가 4곳(WHERE·INSERT·conflict target·envelope)에 흐르므로 declarative `table` 슬롯으로는 컴포즈 안 됨. caller(4개 tri narrative-server)는 *(name, logTag, schema, maxTokens, buildUserContent, findCached, insertCache)* config 한 객체만 제공 — prompt 텍스트(분량 문구·JSON 스키마 예시·schoolSpecific 예시)는 `buildUserContent(frame, school)` 콜백 뒤에 묻혀 prompt 변형 시 factory 무수정. retry/budget/spend 정책 시퀀스 192줄 복제가 소멸. `createSajuTriCache` (frame 캐시) + `createNarrativeHandler` (route) 와 함께 삼국 사주의 thick-adapter/thin-factory 삼총사를 이룬다. Design spec: `docs/superpowers/specs/2026-06-19-saju-narrative-cache-deepening.md`.
