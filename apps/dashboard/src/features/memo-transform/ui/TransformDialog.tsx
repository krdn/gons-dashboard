        ...
        if (r.kind === "ok") {
          ...
          setNotice(
            r.kind === "too-short"
              ...
              : r.kind === "not-found"
                ? "메모를 찾을 수 없습니다."
                : r.kind === "failed" && r.reason === "model-unavailable"
                  ? "선택한 모델을 현재 프록시 인증으로 사용할 수 없습니다. 설정에서 다른 모델을 선택해 주세요."
        ...
        {phase === "pick" && (
          <>
            <p className="mb-3 text-xs text-neutral-400">
              스타일을 선택하면 현재 정리본을 기준으로 변환합니다. 텍스트는
              서버로 전송됩니다.
            </p>
          ...
          <>
            {existingPresets.includes(preset) && (
              <p className="mb-2 text-xs text-amber-600">
                기존 {presets.find((x) => x.slug === preset)?.label ?? preset}{" "}
                정리본을 교체합니다.
              </p>
            )}
            {truncated && (
              <p className="mb-2 text-xs text-neutral-500">
                원문이 길어 앞부분(4,000자)만 변환되었습니다.
              </p>
            )}
            <textarea
              value={content}
