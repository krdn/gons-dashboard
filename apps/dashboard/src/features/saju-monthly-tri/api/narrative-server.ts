...
async function callMonthlyLlmAndParseWithRetry(
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    try {
      ...
      console.error(
        `[saju-monthly-narrative] LLM_CALL_FAIL model=${modelId} school=${school} attempt=${attempt}: ${err instanceof Error ? err.message : String(err)}`,
      );
      throw err;
    }
    ...
    try {
      ...
      if (err instanceof ZodError) {
        console.error(
          `[saju-monthly-narrative] ZOD_FAIL model=${modelId} school=${school} attempt=${attempt} stop=${stopReason} text_len=${text.length}: ${err.issues.map((i) => `${i.path.join(".")}: ${i.message}`).join("; ")}`,
        );
        if (attempt === 2) throw err;
      } else {
        console.error(
          `[saju-monthly-narrative] JSON_PARSE_FAIL model=${modelId} school=${school} attempt=${attempt} stop=${stopReason} text_len=${text.length} text_head=${JSON.stringify(text.slice(0, 200))}: ${err instanceof Error ? err.message : String(err)}`,
        );
        throw err;
      }
    }
