// monthly 의 4학파 합의(agreement) 표시 — server-safe, no client hooks.
//
// YearlyCrossCheckBadge 미러. 차이: targetMonth 도 헤더에 함께 표시.
//
// 정책 (yearly 와 동일):
...
export function MonthlyCrossCheckBadge({ triNation }: Props) {
  const { targetYear, targetMonth, crossCheck } = triNation;
  ...
  return (
    <div className="rounded-xl border border-[var(--color-hairline)] bg-[var(--color-surface-2)] p-3 text-sm space-y-1">
      <div className={label.className}>
        {label.icon} {targetYear}년 {targetMonth}월 — {label.text}
      </div>
