...
describe("searchMemos", () => {
  ...
  it("제목·원문·정리본 어디서든 매칭한다", async () => {
    ...
    expect((await searchMemos(USER_ID, "해지")).map((m) => m.title)).toEqual(["인터넷 해지"]);
    expect((await searchMemos(USER_ID, "위약금")).map((m) => m.title)).toEqual(["제목B"]);
    expect((await searchMemos(USER_ID, "페르소나")).map((m) => m.title)).toEqual(["제목C"]);
  });
  ...
  it("토큰 간 AND — 서로 다른 필드에 걸쳐도 한 메모 안에서 모두 일치해야 한다", async () => {
    ...
    const hits = await searchMemos(USER_ID, "LG 위약금");
  ...
  it("LIKE 메타문자는 리터럴로 매칭한다", async () => {
    ...
    const hits = await searchMemos(USER_ID, "100%");
  ...
  it("최신순으로 정렬한다", async () => {
    ...
    const hits = await searchMemos(USER_ID, "공통어");
  ...
  it("빈 쿼리는 빈 결과", async () => {
    ...
    expect(await searchMemos(USER_ID, "   ")).toEqual([]);
  });
});
