...
- Modify: `apps/dashboard/src/widgets/saju-detail/index.ts` (SajuDailyFortune export 제거)
...
- [ ] **Step 5: `widgets/saju-detail/index.ts` 에서 `SajuDailyFortune` export 제거**
...
```bash
git add apps/dashboard/src/app/fortune/[profileId]/page.tsx apps/dashboard/src/widgets/saju-detail/index.ts
git -c commit.gpgsign=false commit -m "feat(saju): daily 탭 페이지 통합 + reading 탭 옛 일진 섹션 제거"
```

---

## Task 14: 옛 `SajuDailyFortune` 컴포넌트 파일 + 통합 테스트 제거

**Files:**
- Delete: `apps/dashboard/src/widgets/saju-detail/ui/SajuDailyFortune.tsx`
- Delete: `apps/dashboard/tests/saju-cron-daily.integration.test.ts`

- [ ] **Step 1: `SajuDailyFortune` 컴포넌트 호출자 재검증**

Run: `grep -rn "SajuDailyFortune" apps/dashboard/src/ --include="*.ts" --include="*.tsx"`
Expected: 검색 결과 없음 (Task 13 에서 모두 정리됨).

- [ ] **Step 2: 컴포넌트 파일 삭제**

Run: `git rm apps/dashboard/src/widgets/saju-detail/ui/SajuDailyFortune.tsx`
Expected: 파일 삭제됨.

- [ ] **Step 3: 옛 cron daily 통합 테스트 삭제**

Run: `git rm apps/dashboard/tests/saju-cron-daily.integration.test.ts`
Expected: 파일 삭제됨.

- [ ] **Step 4: typecheck + lint + 단위 테스트 통과**

Run: `cd apps/dashboard && pnpm typecheck && pnpm lint`
Expected: 0 errors.

Run: `cd apps/dashboard && TEST_DATABASE_URL="postgres://test:test@127.0.0.1:5999/test_dummy" pnpm vitest run`
Expected: 모든 단위 테스트 통과 (통합 테스트는 DB 미연결 시 ECONNREFUSED 인정).

- [ ] **Step 5: Commit**

```bash
git -c commit.gpgsign=false commit -m "chore(saju): 옛 SajuDailyFortune 컴포넌트 + cron-daily 통합 테스트 제거"
```
