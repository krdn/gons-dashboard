...
- **누락 주 백필** (리뷰 확정 결함 반영): due 판정이 최신 창 하나만 보면 실패가
  ...
  누락된 창을 **오래된 순으로 한 실행당 최대 4주** 백필 생성. 상한 컷은 반드시
  오래된 순 — 최신 우선 컷은 커서(max week_end)가 배치 후 구멍을 건너뛰어 4주
  초과 공백의 잔여 주가 영구 스킵된다 (Codex 리뷰 반영). 잔여 주는 다음 실행
  (매일 19:05)이 이어서 소진. push는 현재(최신) 주가 배치에 포함된 경우에만 —
  회복 직후 알림 폭주 방지. 중간 실패는 throw — 생성분까지는 기록돼 다음 실행이
  이어서 재시도.
