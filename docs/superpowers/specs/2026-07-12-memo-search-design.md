...
- **정렬·상한**: `created_at DESC LIMIT 50` — 메모는 시간 지향 데이터라 최신순이 자연스럽다.
  절단 여부는 **LIMIT+1 센티널 조회**로 정확히 판별해 `truncated` 플래그로 반환 —
  정확히 50개 일치일 때 "최근 50개만 표시"가 거짓 표시되는 오류 방지 (리뷰 반영).
...
- **매칭 뷰 자동 선택** (리뷰 반영): 정리본·제목에 일치가 없고 원문·변환본에서만
  일치하면 그 뷰를 초기 표시 — 하이라이트 없는 카드가 "무관한 결과"처럼 보이는 문제 방지.
  구현은 파생 초기 뷰 + 사용자 오버라이드(`userView ?? derived`) 패턴 — effect 없이 순수.
