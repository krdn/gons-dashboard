...
| 갈래 | 출처 | 영속화 |
|------|------|--------|
| 상태/준비도 (mode, deploy on/off, 다음 사이클) | 최신 DB row (mode·deploy) + 서버 시각(다음 사이클) | 불필요 (cycle row에서 파생) |
...
export const autopilotCycles = pgTable("autopilot_cycles", {
  ...
  mode: text("mode").notNull(),               // "shadow" | "autonomous" — cycle 실행 시 args.mode
  deployFlag: text("deploy_flag"),            // "on" | "off" — 저장 시점 cron의 AUTOPILOT_DEPLOY (app env엔 없으므로 호출자가 실어 보냄)
...
- `mode` → **최신 cycle row의 `mode` 컬럼** (cycle 실행 시 `args.mode`로 주입된 실제 값). env 아님 — cycle.workflow.js:31 `args?.mode ?? "shadow"`.
- `deploy on/off` → **cycle row 저장 시점에 함께 기록된 `deployFlag` 컬럼**. ⚠️ `AUTOPILOT_DEPLOY`는 docker-compose.yml의 **cron 서비스에만** 주입됨(line 125) — dashboard app env(line 65-98)엔 없음. 위젯이 `env.AUTOPILOT_DEPLOY`를 읽으면 항상 기본값(off) → 조용히 틀린 배지. 그래서 저장 라우트(=cron 측 호출자)가 deploy 플래그를 row에 실어 보내고 위젯은 row에서 읽는다. row 없으면(이력 0건) "미상" 표시.
- 다음 사이클 D-day → "주 1회 월요일" 가정에서 계산 (서버 RSC, KST). 실제 cron 요일 확정 전 표시 가정 — 상수로 둠.
