...
1. **자동 정리분(commit 8559d78)은 검증 완료** — 머지 가능.
2. **C의 `TickerInput.tsx`, D의 prettier-plugin** 정도가 추가 정리 여지. 사용자 확인 후 별도 PR.
3. **B/E는 barrel·cross-workspace 표면이라 일괄 정리 비권장** — FSD 공개 계약을 의도적으로 좁힐 때만, slice 단위로 검토.
